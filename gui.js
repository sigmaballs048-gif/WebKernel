 ...
 * Controls DOM structures, frame composition pipelines, structural manipulation nodes, layouts, and styles.
 */
...
class WebGUI {
    ...
    _buildDesktopWorkspaceLayout() {
        // Evaluate dynamic fallback configurations from central registries
        ...
        // Bind core dynamic layout interface interactions
    ...
    _wireGlobalUserInteractions() {
        this.root.addEventListener("mousedown", (e) => {
            ...
            if (hitWindow) {
                this.focusWindow(hitWindow.dataset.windowId);
            }
        });

        // Sync visual changes based on shared application notification loops
    ...
    _populateLauncherApplications() {
        ...
        applicationExecutables.forEach(app => {
            ...
            row.onclick = () => {
                this.kernel.process.launchFromFile(app.path)
                    .catch(err => alert(`App Execution Fault: ${err.message}`));
    ...
    _handleIconExecutionRoute(fileRecord) {
        if(fileRecord.path.endsWith(".js")) {
            this.kernel.process.launchFromFile(fileRecord.path)
                .catch(err => alert(`Workflow Error: ${err.message}`));
        } else {
            // Render text preview window directly
            this.createWindow({
    ...
    createWindow(options) {
        ...
        wrapperNode.querySelector(".btn-close").onclick = (e) => {
            e.stopPropagation();
            this.closeWindow(winId);
        };

        if(options.content) {
            wrapperNode.querySelector(".window-body-content").innerHTML = options.content;
        }
    ...
    focusWindow(winId) {
        ...
        if (frame) {
            frame.style.zIndex = this.layerIndexCounter++;
        }
    }
    ...
    toggleThemeEngine() {
        ...
        this.windows.forEach(winFrame => {
            if(this.activeTheme === "light") {
                winFrame.classList.add("light-mode-vars");
            } else {
                winFrame.classList.remove("light-mode-vars");
            }
        });
